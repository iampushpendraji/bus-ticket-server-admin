...
const pool = mysql.createPool(my_sql_db_config);
...
const redis_cli = new Redis(redis_db_config);


// Function to establish database connection (MySQL && Redis)
const connect_db = new Promise(async (res, rej) => {
  try {
    const connection = await pool.getConnection();  // Checking connection of MySQL
